...
// https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&id=27433442&retmode=json
...
function pmid_metadata($reference, $pmid)
{
	...
	if ($xml != '')
	{
		...
		if (!isset($reference->PMC))
		{
			...
			foreach($nodeCollection as $node)
			{
				$reference->PMC = $node->firstChild->nodeValue;
				$reference->{'alternative-id'}[] = $reference->PMC;
			}
		}	
	}
