...
$urls = array(
'http://www.europeanjournaloftaxonomy.eu/index.php/ejt/article/view/306'
);

foreach ($urls as $url)
{

	//$html = file_get_contents('306.html');
	$html = get($url);
	...
	foreach ($ps as $p)
	{
		...
		if (!$matched)
		{
			if (preg_match('/(?<authorstring>.*)\s+(?<year>[0-9]{4})[a-z]?\.\s*(?<title>.*)\.\s+(?<journal>.*)\s+(?<volume>\d+):\s+(?<spage>\d+)–(?<epage>\d+)/u', $p->innertext, $m))
			{
				//print_r($m);
				$matched = true;
			}
